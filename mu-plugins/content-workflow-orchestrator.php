...
add_action('transition_post_status', function($new, $old, $post){
  ...
  // Map primary category to hub page
  ...
  if ( $hub_id ) {
    ...
    // Cache-safe: purge hub page cache if common cache plugins exist
    if ( function_exists('rocket_clean_post') ) rocket_clean_post( $hub_id );
...
// ---- Hub mapping: primary category to hub page ----
function cwo_map_primary_category_to_hub( $post_id ){
  // Strategy: map primary category slug to a hub Page by slug OR ACF field.
  // 1) Try ACF page field on the category: hub_page_id
  $terms = get_the_terms($post_id, 'category');
  if ( $terms && ! is_wp_error($terms) ) {
    usort($terms, fn($a,$b)=> $a->term_id <=> $b->term_id);
    $primary = $terms[0];
    if ( function_exists('get_field') ) {
      $hub_id = (int) get_field('hub_page_id', 'category_' . $primary->term_id);
      if ( $hub_id ) return $hub_id;
    }
    // 2) Fallback: if a Page exists with same slug as category, treat it as hub
    $page = get_page_by_path( $primary->slug );
    if ( $page && 'page' === $page->post_type ) return (int) $page->ID;
  }
  return 0;
}
